...
class BeepPlayer {
    ...
    private var convertedBuffer:AVAudioPCMBuffer? = nil
    init(frequency: Float, duration: TimeInterval) {
        ...
        guard let convertedBuffer = AVAudioPCMBuffer(pcmFormat: outputFormat, frameCapacity: AVAudioFrameCount(frameCount)) else {
